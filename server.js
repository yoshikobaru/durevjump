...
const bot = new Telegraf(process.env.JUMP_BOT_TOKEN);
// WebApp URL
const webAppUrl = 'https://durovjump.ru';
...
const options = {
    key: fs.readFileSync('/etc/letsencrypt/live/durovjump.ru/privkey.pem'),
    cert: fs.readFileSync('/etc/letsencrypt/live/durovjump.ru/fullchain.pem')
};
...
const server = https.createServer(options, async (req, res) => {
  ...
  if (routes[method] && routes[method][pathname]) {
    ...
    let filePath = path.join(__dirname, '..', 'durevjump', req.url === '/' ? 'main.html' : req.url);
    serveStaticFile(filePath, res);
  }
});

const httpsPort = 6000;
const httpPort = 6001;

server.listen(httpsPort, () => {
  console.log(`HTTPS Сервер запущен на порту ${httpsPort}`);
  console.log('Telegram бот запущен');
  console.log(`HTTPS Сервер запущен на https://durovjump.ru`);
});
